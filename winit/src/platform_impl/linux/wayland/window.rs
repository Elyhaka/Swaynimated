...
pub struct Window {
    ...
    frame: Option<Arc<Mutex<SWindow<ConceptFrame>>>>,
...
impl Window {
    pub fn new<T>(
        ...
        let frame = if pl_attribs.init_with_shell {
        ...
        let frame = frame.map(|frame| Arc::new(Mutex::new(frame)));
        ...
        evlp.store.lock().unwrap().windows.push(InternalWindow {
            ...
            frame: frame.as_ref().map(Arc::downgrade),
            current_dpi: 1,
            new_dpi: None,
        });
    ...
    pub fn set_title(&self, title: &str) {
        if let Some(frame) = &self.frame {
            frame.lock().unwrap().set_title(title.into());
        }
    }
    ...
    pub fn set_resizable(&self, resizable: bool) {
        if let Some(frame) = &self.frame {
            frame.lock().unwrap().set_resizable(resizable);
        }
    }
    ...
    pub fn set_decorations(&self, decorate: bool) {
        if let Some(frame) = &self.frame {
            frame.lock().unwrap().set_decorate(decorate);
    ...
    pub fn set_maximized(&self, maximized: bool) {
        if let Some(frame) = &self.frame {
            if maximized {
                frame.lock().unwrap().set_maximized();
            } else {
                frame.lock().unwrap().unset_maximized();
            }
        }
    }
    ...
    pub fn set_fullscreen(&self, fullscreen: Option<Fullscreen>) {
        if let Some(frame) = &self.frame {
            match fullscreen {
                ...
                Some(Fullscreen::Borderless(_)) => unreachable!(),
                None => frame.lock().unwrap().unset_fullscreen(),
            }
        }
    }
    ...
    pub fn set_theme<T: Theme>(&self, theme: T) {
        if let Some(frame) = &self.frame {
            frame.lock().unwrap().set_theme(theme)
        }
    }
...
struct InternalWindow {
    ...
    frame: Option<Weak<Mutex<SWindow<ConceptFrame>>>>,
    current_dpi: i32,
    new_dpi: Option<i32>,
}
...
impl WindowStore {
    ...
    pub fn new_seat(&self, seat: &wl_seat::WlSeat) {
        for window in &self.windows {
            if let Some(w) = window.frame.as_ref().and_then(|f| f.upgrade()) {
    ...
    {
        for window in &mut self.windows {
            let opt_arc = window.frame.as_ref().and_then(|f| f.upgrade());
